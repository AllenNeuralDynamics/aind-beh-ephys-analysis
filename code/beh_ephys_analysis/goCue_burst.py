...
def burst_analysis(session, data_type, units = None):
    ...
    for unit_id in units:
        ...
        if unit_tbl[unit_tbl['unit_id']==unit_id]['p_max'].values[0]<0.5:
            continue
        # print(unit_id)
        ...
        isi_spikes = np.diff(spike_times_curr)
        edges = np.logspace(np.log10(np.nanmin(isi_spikes) + 0.00000001), np.log10(np.nanmax(isi_spikes)), 50)
        ax.hist(isi_spikes, bins=edges, color='k', alpha = 0.5, density=True)
        # set xlabel to log
        ax.axvline(0.002, color='k', linestyle='--')
        ...
        if len(df)>=10:
            ...
            up_bound = np.nanpercentile(isi_color_code[~np.isnan(isi_color_code)], 95)
            low_bound = np.nanpercentile(isi_color_code[~np.isnan(isi_color_code)], 5)
            ...
            ax.hist(isi_list['isi'].values, bins=edges, color='r', alpha=0.5, density=True)
...
if __name__ == '__main__':
    ...
    # save_process('behavior_791691_2025-06-25_14-06-10', 'curated')
    # burst_analysis('behavior_751766_2025-02-11_11-53-38', 'curated', units = [46])
