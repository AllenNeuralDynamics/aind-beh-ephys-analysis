...
def plot_licks_from_video(session):
    ...
    save_file_selection = os.path.join(session_dir['beh_fig_dir'], f"{session}_lick_feature_relationships.pdf")
    ...
    _, ax, _ = plot_raster_rate(session_licks['start_time_session'].values, session_df['goCue_start_time'].values, np.arange(len(session_df)), edges, ['early', 'late'], color_map, fig, gs[4], color_code=session_licks['duration'].values, color_clip = [0, 0.20])
    ax.set_title('Video Licks')

    plt.tight_layout()

    save_file_raster = os.path.join(session_dir['beh_fig_dir'], f"{session}_lick_raster_comparison.pdf")
    ...
    save_file_space = os.path.join(session_dir['beh_fig_dir'], f'{session}_lick_feature_space.pdf')
    fig.savefig(save_file_space, dpi=300)
    # close fig
    plt.close(fig)

    # combine all pdfs into one
    pdf_files = [save_file_selection, save_file_raster, save_file_space]
    combined_pdf_file = os.path.join(session_dir['beh_fig_dir'], f'{session}_lick_video_analysis_combined.pdf')
    merger = PdfMerger()
