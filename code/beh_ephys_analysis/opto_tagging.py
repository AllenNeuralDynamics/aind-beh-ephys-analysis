...
def max_index_ndarray(arr):
    ...
    return max_nd_index 
# %%
def opto_plotting_unit(unit_id, spike_times, waveform, opto_df, opto_info, dim_1 = 'powers', resp_thresh=0.8, lat_thresh=0.015):
    # calculate baseline
    ...
    return fig
#%%
def opto_plotting_session(session, target, resp_thresh=0.8, lat_thresh=0.015):
    session_dir = session_dirs(session)
    ...
    #     if pass_qc[unit_id]:   
        fig = opto_plotting_unit(unit_id, spiketimes[unit_id], waveforms[unit_id], opto_df, opto_info, dim_1 = 'powers', resp_thresh=resp_thresh, lat_thresh=lat_thresh)
...
if __name__ == "__main__":
    ...
    opto_plotting_session(session, target, resp_thresh=resp_thresh, lat_thresh=lat_thresh)
