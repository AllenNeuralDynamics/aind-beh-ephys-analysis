...
from utils.ephys_functions import plot_rate, get_spike_matrix, correlate_nan, load_drift, fitSpikeModelG_CI
...
def pupil_analysis_session(session, plot_licks=False, plot=True,
    ...
    qual_mask_all = (pupil_data['time_conf_trial'] > time_conf_threshold) & (pupil_data['dia_conf_trial'] > dia_conf_threshold)
    qual_mask_choice = qual_mask_all[beh_df['animal_response'].values != 2]
    
    # remove lick trains with in 5s from a bad quality trial
    bad_trial_starts = beh_df['goCue_start_time'].values[~qual_mask_all]
    lick_mask = np.ones_like(lick_data['lick_trains_all']['train_starts'], dtype=bool)
    lick_mask_licks_L = np.ones_like(lick_data['lick_trains_L']['train_starts'], dtype=bool)
    lick_mask_licks_R = np.ones_like(lick_data['lick_trains_R']['train_starts'], dtype=bool)
    for bad_start in bad_trial_starts:
        lick_mask[np.abs(lick_data['lick_trains_all']['train_starts'] - bad_start) < 5] = False
        lick_mask_licks_L[np.abs(lick_data['lick_trains_L']['train_starts'] - bad_start) < 5] = False
        lick_mask_licks_R[np.abs(lick_data['lick_trains_R']['train_starts'] - bad_start) < 5] = False
    lick_data['lick_trains_all'] = {k: v[lick_mask] for k, v in lick_data['lick_trains_all'].items() if len(v) == len(lick_mask)}
    lick_data['lick_trains_L'] = {k: v[lick_mask_licks_L] for k, v in lick_data['lick_trains_L'].items() if len(v) == len(lick_mask_licks_L)}
    lick_data['lick_trains_R'] = {k: v[lick_mask_licks_R] for k, v in lick_data['lick_trains_R'].items() if len(v) == len(lick_mask_licks_R)}
    ...
    session_df_z = session_df_z[qual_mask_choice].reset_index(drop=True)
    ...
    out_all, bins = bin_timeseries_around_align(
        ts={'time': pupil_data['pupil_times'], 'value': pupil_data['pupil_diameter_z']},
        align_times=beh_df['goCue_start_time'].values[qual_mask_all],
        step_size=0.1,
        bin_size=bin_size,
        t_start=-2,
        t_stop=4
    ...
    if plot: 
        ...
        _, ax = plot_rate(out, bins, session_df['svs'].values[qual_mask_choice], [-1, 0.5, 2], ['stay', 'switch'], colormap, fig, gs[0,0])
        ax.set_title('stay vs switch')
        _, ax = plot_rate(out, bins, session_df['outcome'].values[qual_mask_choice], [-1, 0.5, 2], ['no reward', 'reward'], colormap, fig, gs[0,1])
        ax.set_title('no reward vs reward')
        _, ax = plot_rate(out, bins, session_df['choice'].values[qual_mask_choice], [-1, 0.5, 2], ['left', 'right'], colormap, fig, gs[0,2])
        ...
        _, ax = plot_rate(out[session_df[qual_mask_choice]['choice'].values==1, :], bins, session_df[(session_df['choice']==1)&qual_mask_choice]['outcome'], [-1, 0.5, 2], ['no reward', 'reward'], colormap, fig, gs[0, 3])
        ax.set_title('Right choice: no reward vs reward')
        # reward vs no reward on left side
        _, ax = plot_rate(out[session_df[qual_mask_choice]['choice'].values==0, :], bins, session_df[(session_df['choice']==0)&qual_mask_choice]['outcome'], [-1, 0.5, 2], ['no reward', 'reward'], colormap, fig, gs[1, 3])
        ...
        if lm_results['regressors'] is not None:
        ...
        if lm_results_all is not None:
            if lm_results_all['regressors'] is not None:
...
def plot_unit_pupil_correlation(session, bin_size = 5, step_size = 0.1, opto_only=True, bin_size_short=0.5, win_length=10, plot = True, time_conf_threshold=0.9, dia_conf_threshold=0.95):
    ...
    # set those close to trial times not pass threshold to nan
    qual_mask_all = (pupil_data['time_conf_trial'] > time_conf_threshold) & (pupil_data['dia_conf_trial'] > dia_conf_threshold)
    for trial_ind, go_cue in enumerate(sesssion_df['goCue_start_time'].values):
        if not qual_mask_all[trial_ind]:
            mask = (pupil_times >= go_cue-3) & (pupil_times <= go_cue +3)
            pupil_values[mask] = np.nan
    ...
    for i, row in unit_tbl.iterrows():
        ...
        if plot:
            ...
            if np.sum(~np.isnan(pupil_baseline)) > 10:
