"""
Step 2 of figure preparation pipeline: Combine antidromic identification results.
...
Prerequisites:
    ...
    Data requirements:
    - Per-session *_antidromic_results.pkl files must exist in opto_dir_curated/
    - Generated by analyze_antidromic_responses() in antidromic_analysis_session.py

Pipeline Position:
    Script #2 in sequence.txt - Run after Step 1 completes successfully.

Purpose:
    Aggregates antidromic analysis results from all sessions into a single DataFrame.
    Combines collision test results, response latencies, jitter measurements, and
    tier classifications for all opto-tagged units across the dataset.

Input:
    - Per-session antidromic analysis files (*_antidromic_results.pkl)
    - Combined unit table from Step 1

Output:
    - combined_antidromic_tbl.pkl: DataFrame with antidromic metrics per unit
    - Includes: collision p-values, latencies, jitter, tier assignments per stimulation site

Usage:
    Run after make_combined_unit_tbl.py completes.
"""
