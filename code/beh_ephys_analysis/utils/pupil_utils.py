...
def linreg_remove_outliers(A, B, z=3.5):
    """
    ...
    # refit on inliers
    m2, c2 = np.polyfit(x[inliers_sub], y[inliers_sub], 1)
...
def load_pupil(session): # = 'behavior_ZS062_2021-04-02_19-08-52'
    ...
    if len(pupil_file)==0:
        ...
        if len(pupil_data['cueFT'][0].squeeze()) == len(raw_df['goCue_start_time'].values):
            ...
            m, c, inlier_mask, outlier_idx, resid = linreg_remove_outliers(np.diff(pupil_data['cueFT'][0].squeeze()), np.diff(raw_df['goCue_start_time'].values), z=3.5)

                
            interp_func = interp1d(pupil_data['cueFT'][0].squeeze(), raw_df['goCue_start_time'].values, 
                                    bounds_error=False, fill_value="extrapolate")
            best_coeff = 1
            go_cue_frames = pupil_data['cueFT'][0].squeeze()
            ...
            if len(cueFT_diff) > len(goCue_diff):
                ...
                interp_func = interp1d(matched_FT, raw_df['goCue_start_time'].values,
                                       bounds_error=False, fill_value="extrapolate")
                go_cue_frames = matched_FT
                ...
                best_coeff = np.max(corrs)
                interp_func = interp1d(pupil_data['cueFT'][0].squeeze(), matched_cue,
                                       bounds_error=False, fill_value="extrapolate")
                go_cue_frames = pupil_data['cueFT'][0].squeeze()
                qual_ind = pupil_data['qualInd'][0]
